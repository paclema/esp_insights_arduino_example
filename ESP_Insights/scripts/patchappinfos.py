...
def PatchAppInfos(source, target, env):
    ...
    shutil.copy(img_fpath, img_fpath + ".old")
...
def install_patch_app_infos(env):
    patch_action = env.AddCustomTarget(
        ...
        PatchAppInfos,
        title="Engrave image with app-infos",
    ...
    def DumpAppInfos(source, target, env):
        img_fpath = source[0].path
        print("Image path:", img_fpath)
        with io.open(img_fpath, "rb") as fd:
            img = load_and_verify_image(fd)

        # print( "is_hashed: ",f"True - {hash.hex()}" if img.is_hashed else "False")
        print( "is_hashed: ",f"True" if img.is_hashed else "False")
